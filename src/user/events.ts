...
export type UserSignals = {
  // READ
  fetch_user: Signal<{ identity: string; authToken?: string }>;
  fetch_user_favourites: Signal<{ userId: string; authToken: string }>;
  search_users: Signal<{
    userIds?: string[];
    authToken?: string;
  }>;
  ...
  sync_user_rating: Signal<{ userId: string; authToken: string }>;
  ...
  delete_user: Signal<{ userId: string; authToken: string }>;
};

export type UserMessages = {
  // READ
  user_fetched: Message<{ user: User }>;
  fetch_user_failed: ErrorMessage;
  user_favourites_fetched: Message<{ productIds: string[] }>;
  ...
  user_rating_changed: Message<{ userId: string; rating: number }>;
  ...
  user_deleted: Message<{ userId: string }>;
  delete_user_failed: ErrorMessage<{ userId: string }>;
};
