...
export type ListingSignals = {
  // READ
  fetch_listing: Signal<{ id: string }>;
  fetch_listing_address: Signal<{ listingId: string; authToken: string }>;
  fetch_listing_history: Signal<{ listingId: string; authToken: string }>;
  fetch_listing_counts: Signal<{ productIds?: string[]; gameIds?: string[] }>;
  fetch_suggestions: Signal<{ userId: string; authToken: string }>;
  ...
  delete_listing: Signal<{ authToken: string; listingId: string }>;
  sync_listing_rating: Signal<{ id: string; authToken: string }>;
  sync_listing_status: Signal<{ listingId: string; authToken: string }>;
...
export type ListingMessages = {
  // READ
  listing_fetched: Message<{ listing: Listing }>;
  fetch_listing_failed: ErrorMessage<{ id: string }>;
  listing_address_fetched: Message<{ name: string; address: Address }>;
  fetch_listing_address_failed: ErrorMessage<{ listingId: string }>;
  listing_history_fetched: Message<{ history: AuditItem[] }>;
  fetch_listing_history_failed: ErrorMessage<{ listingId: string }>;
  listing_counts_fetched: Message<{
    counts: Array<{ productId: string; count: number }>;
  }>;
  fetch_listing_counts_failed: ErrorMessage<{
    productIds?: string[];
    gameIds?: string[];
  }>;
  suggestions_fetched: Message<{ listings: Listing[] }>;
  ...
  verify_listing_failed: ErrorMessage<{ listingId: string }>;
};
